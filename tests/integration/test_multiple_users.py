...
from htf_engine.errors.exchange_errors.fok_insufficient_liquidity_error import (
    FOKInsufficientLiquidityError,
)
from htf_engine.errors.exchange_errors.post_only_violation_error import (
...
class TestExchange:
    def _nice_snapshot(self, ob):
        ...
        for price, orders in snap["bids"]:
            bids[price] = sum(
                o[3] for o in orders if o not in ob.cancelled_orders
            )  # o[3] is qty
    ...
    def test_exchange(self, exchange, u1, u2, u3):
        ...
        id_user1_order3 = exchange.modify_order(
            u1.user_id, inst, id_user1_order2, 70, 20
        )
        ...
        id_user1_order4 = exchange.modify_order(
            u1.user_id, inst, id_user1_order3, 50, 20
        )

        assert (
            id_user1_order3 == id_user1_order4
        )  # order_id should be the same since only quantity decreased
        ...
        assert (
            str(e1.value)
            == "[ORDER_EXCEEDS_POSITION_LIMIT] Rejected Order: Cannot place buy order for 50x Stock A as it exceeds the quota of 0."
        )
        ...
        assert (
            str(e2.value)
            == "[ORDER_EXCEEDS_POSITION_LIMIT] Rejected Order: Cannot place sell order for 50x Stock A as it exceeds the quota of 0."
        )
        ...
        assert u1.get_realised_pnl() == 0
        assert (
            exchange.get_user_unrealised_pnl(u1.user_id) == 0
        )  # Prices haven't moved yet
        assert (
            exchange.get_user_exposure(u1.user_id) == 500
        )  # User 1 owns 50 shares @ $10 each
        assert u1.get_cash_balance() == 4490
        assert u1.get_outstanding_buys() == {}
        assert u1.get_outstanding_sells() == {inst: 50}
        assert u1.get_remaining_quota(inst) == {
            "buy_quota": 50,
            "sell_quota": 100,
        }  # Can sell up to 150 shares now, but 50 quota already used in existing order

        assert u2.positions == {inst: -50}
        assert u2.get_realised_pnl() == 0
        assert (
            exchange.get_user_unrealised_pnl(u2.user_id) == 0
        )  # Prices haven't moved yet
        assert (
            exchange.get_user_exposure(u2.user_id) == 500
        )  # User 2 owns -50 shares @ $10 each
        ...
        assert (
            u1.get_realised_pnl() == 250
        )  # User 1 sold 25 shares, earning $10 profit per share
        assert (
            exchange.get_user_unrealised_pnl(u1.user_id) == 250
        )  # User 1 has 25 shares with cost basis of $10 (current price = $20)
        ...
        assert u1.get_remaining_quota(inst) == {
            "buy_quota": 75,
            "sell_quota": 100,
        }  # Can sell up to 125 shares now, but 25 quota already used in existing order

        assert u2.positions == {inst: -50}
        assert u2.get_realised_pnl() == 0
        assert (
            exchange.get_user_unrealised_pnl(u2.user_id) == -500
        )  # User 2 is short 50 shares with unrealised loss of $10 per share
        assert (
            exchange.get_user_exposure(u2.user_id) == 1000
        )  # User 2 owns -50 shares @ $20 each
        assert u2.get_cash_balance() == 5490
        assert u2.get_outstanding_buys() == {}
        assert u2.get_outstanding_sells() == {}
        assert u2.get_remaining_quota(inst) == {"buy_quota": 150, "sell_quota": 50}
        ...
        assert (
            exchange.get_user_unrealised_pnl(u3.user_id) == 0
        )  # Prices haven't moved yet
        assert (
            exchange.get_user_exposure(u3.user_id) == 500
        )  # User 3 owns 25 shares @ $20 each
        ...
        assert (
            last_price == 20
        )  # The trade still goes through at $20, which is the best ask
        assert last_quantity == 15
        assert exchange.balance == 60

        assert u1.positions == {inst: 10}  # User 1 sold all their shares
        assert (
            u1.get_realised_pnl() == 400
        )  # User 1 sold another 15 shares, earning $10 profit per share
        assert (
            exchange.get_user_unrealised_pnl(u1.user_id) == 100
        )  # User 1 is holding 10 shares with cost basis of $10 (current price = $20)
        ...
        assert (
            exchange.get_user_unrealised_pnl(u2.user_id) == -500
        )  # User 2 is still short 50 shares with unrealised loss of $10 per share
        assert (
            exchange.get_user_exposure(u2.user_id) == 1000
        )  # User 2 still owns -50 shares @ $20 each
        assert u2.get_cash_balance() == 5490
        assert u2.get_outstanding_buys() == {}
        assert u2.get_outstanding_sells() == {}
        assert u2.get_remaining_quota(inst) == {"buy_quota": 150, "sell_quota": 50}
        ...
        assert (
            exchange.get_user_exposure(u3.user_id) == 800
        )  # User 3 owns 40 shares @ $20 each
        ...
        assert (
            u1.get_realised_pnl() == 450
        )  # User 1 sold another 5 shares, earning $10 profit per share
        assert (
            exchange.get_user_unrealised_pnl(u1.user_id) == 50
        )  # User 1 is holding 10 shares with cost basis of $10 (current price = $20)
        ...
        assert (
            exchange.get_user_exposure(u3.user_id) == 900
        )  # User 3 owns 40 shares @ $20 each
        ...
        assert (
            exchange.get_user_exposure(u3.user_id) == 1000
        )  # User 3 owns 50 shares @ $20 each
        ...
        assert (
            str(e3.value)
            == "[ORDER_EXCEEDS_POSITION_LIMIT] Rejected Order: Cannot place buy order for 75x Stock A as it exceeds the quota of 50."
        )
        ...
        assert (
            str(e4.value)
            == "[POST_ONLY_VIOLATION] Rejected Order: Post-only order would take liquidity and was rejected."
        )
        ...
        assert (
            last_price == 100
        )  # The trade still goes through at $100, which is the best ask
        ...
        assert (
            u2.get_realised_pnl() == -2700
        )  # User 2 bought 30 shares at $100 each, incurring $90 loss per share
        assert (
            exchange.get_user_unrealised_pnl(u2.user_id) == -1800
        )  # User 2 is still short -20 shares with cost basis of $10 (current price = $100)
        ...
        assert (
            u3.get_realised_pnl() == 2400
        )  # User 3 sold 30 shares at $100 each, earning $80 profit per share
        assert (
            exchange.get_user_unrealised_pnl(u3.user_id) == 1600
        )  # User 3 still holds 20 shares with cost basis of $20 (current price = $100)
